...
const notificationType = "inactive_no_login";

function getStudents() {
    const sql = "SELECT id, gcm_reg_id, created_at from pre_login_onboarding where created_at >= date_sub(CURRENT_TIMESTAMP, INTERVAL 2 MINUTE) AND created_at < date_sub(CURRENT_TIMESTAMP, INTERVAL 1 MINUTE) AND is_converted = 0 AND gcm_reg_id <> '' ORDER BY created_at DESC";
...
async function start(job) {
    ...
    const firebaseEventTag = "NEWLOGIN2";
