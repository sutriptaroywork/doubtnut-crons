...
const plans = [
    {
        ...
        hourInterval: 12, // in Hours
        offsetInterval: 3, // in Hours
        notificationInfo: {
            event: "video",
            title: "Improve your SAT Score!",
...
async function start(job) {
    ...
    try {
        for (let i = 0; i < plans.length; i++) {
            ...
            const upperInterval = plan.dayInterval * 24 + plan.offsetInterval;
            ...
            for (let j = 0; j < users.length; j++) {
                ...
                if ((!(j % NOTIFS_PER_ITERATION) || (j === (users.length - 1))) && sendTo.length) {
                    // console.log("i", i + 1, "j", j, "SENDING NOTIF FOR VIDEO VIEW:", sendTo.length, notificationInfo.firebase_eventtag);
                    // eslint-disable-next-line no-await-in-loop
