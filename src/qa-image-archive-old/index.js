...
const db = "doubtnut";
const collection = "qa_image_archive_logs_old";
...
async function getQAImages() {
    ...
    const sql = `select question_id, question_image from user_questions_archive_old where student_id > 100 and (question_image like 'upload%' or question_image like '8400%' or question_image like '600%' or question_image like 'bot%') and question_id>${lastId} and is_answered=0 and is_text_answered=0 order by question_id limit ${batchsize}`;
...
async function processImages(firstId, lastId, images) {
    ...
    if (status.length) {
        ...
        await aws.s3.putObject({
            Bucket: "doubtnut-user-uploads/archive-old",
